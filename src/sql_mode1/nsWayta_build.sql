...
--
SELECT tStore.upsert(term, tlib.nsget_nsid('wayta-code'), jinfo, false, NULL::int, false, NULL::boolean, 1)
...
--
SELECT tStore.upsert(term, tlib.nsget_nsid('wayta-en'), jinfo, false, NULL::int, false, NULL::boolean, 1)
FROM tlib.tmp_waytaff
...
--
SELECT tStore.upsert(term, tlib.nsget_nsid('wayta-es'), jinfo, false, NULL::int, false, NULL::boolean, 1)
FROM tlib.tmp_waytaff
...
) SELECT tStore.upsert(form, tlib.nsget_nsid('wayta-pt'), NULL, true, NULL::int, false, NULL::boolean, 1)
  FROM uforms
...
) SELECT tStore.upsert(form, tlib.nsget_nsid('wayta-pt'), jinfo, true, NULL::int, false, NULL::boolean, 1)
  FROM uforms
  WHERE form NOT IN (SELECT term FROM tstore.term WHERE (fk_ns&tlib.basemask('wayta-pt'))::boolean)
; -- ~19000 rows
