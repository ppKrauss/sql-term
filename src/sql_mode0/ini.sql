...
CREATE TABLE term0.ns( -- namespace
  ...
  kx_regconfig regconfig DEFAULT 'english'::regconfig,  -- can be changed by app as f(lang,env)
...
INSERT INTO term0.ns(label,description,lang,kx_regconfig) VALUES ('teste','first test','pt','portuguese'::regconfig);
...
CREATE INDEX term_idx ON term0.term(term);  -- important
CREATE INDEX term_metaphone_idx ON term0.term(kx_metaphone); -- important
CREATE INDEX term_tsvector_idx ON term0.term(kx_tsvector); -- need to index?
...
BEGIN
	...
	SELECT kx_regconfig INTO regconf FROM  term0.ns WHERE nsid=NEW.fk_ns;
	NEW.kx_tsvector := to_tsvector(regconf, words[1]);
	RETURN NEW;END;
$f$ LANGUAGE plpgsql;
...
RETURNS text AS $f$
	SELECT 	 trim( string_agg(metaphone(w,$2),$3) ,$3)
...
--
	...
	SELECT CASE WHEN $2='e'  or $2='=' THEN (  -- exact
			...
			WHERE fk_ns=$4 AND kx_tsvector @@ to_tsquery(
				$5,
				LOWER( TRIM( regexp_replace($1, E'[\\s;\\|]+', $2, 'g'), $2 ) )
			) LIMIT $3 
		) END
	;
$f$ LANGUAGE sql;
...
-- Only didactic illustration, it is not a good algorithm.
--
	text,                      -- 1. query string (usual term or term-fragments)
...
DECLARE
  items text[];
BEGIN
	items := term0.search($1,'=',$2,$3); -- exact
	IF array_length(items,1)=1 THEN
		RETURN items;
	END IF;
	items := term0.search($1,'&',$2,$3); -- all words, any order
	IF array_length(items,1)>0 THEN
		RETURN items;
	END IF;
	items := term0.search($1,'p',$2,$3); -- first words, exact order
	IF array_length(items,1)>0 THEN
		RETURN items;
	END IF;
	items := term0.search_metaphone($1,'=',$2,$3,$4); -- exact metaphone
	IF array_length(items,1)>0 THEN
		RETURN items;
	END IF;
	items := term0.search_metaphone($1,'&',$2,$3,$4); -- all metaphones, any order
	IF array_length(items,1)>0 THEN
		RETURN items;
	END IF;
